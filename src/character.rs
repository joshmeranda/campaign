...
impl App {
            ...
            match key.code {
                ...
                KeyCode::Up => match self.active_tab {
                    ...
                    Tab::Notes => self
                        ...
                        .scroll(ratatui::widgets::ScrollDirection::Backward),
                },
    ...
    fn render_header_name(&self, frame: &mut Frame, area: Rect) {
        ...
        let [temp_health, health] = area
            .centered(Length(36), Length(6))
            .layout(&Layout::vertical([Length(1), Length(1)]).flex(layout::Flex::SpaceEvenly));
        ...
        let temp_health_str = if self.status.temp_hp > 17 {
            ...
            (0..self.status.temp_hp)
                .map(|_| "█")
                .collect::<Vec<&str>>()
                .join(" ")
        };

        frame.render_widget(Paragraph::new(temp_health_str.light_blue()), temp_health);
    ...
    fn render_header(&self, frame: &mut Frame, area: Rect) {
        let [name, info] = area.layout(&Layout::horizontal([Length(40), Fill(1)]).spacing(1));
    ...
    fn render_ability(&self, frame: &mut Frame, area: Rect, ability: &Ability) {
        ...
        ability.name.proficiencies().into_iter().for_each(|p| {
            if ability.proficiencies.contains(&p) {
                lines.push(Line::from(Span::styled(
                    p.clone(),
                    Style::default().bg(Color::White).black(),
                )));
            } else {
        ...
        frame.render_widget(
            Paragraph::new(text).alignment(Alignment::Center).block(
                Self::default_block().title(format!(" {} : {} ", ability.name, modifier).bold()),
            ),
            area,
        );
    }
    ...
    fn render_class_abilities(
        ...
        let [rages, spell_slots] = area.layout(
            &Layout::vertical([
        ...
        for (i, coin) in coins.iter().enumerate() {
            ...
            frame.render_widget(
                Paragraph::new(self.status.coins[i].to_string())
                    .block(Self::default_block().title(title)),
                *coin,
            );
        }
    }
    ...
    fn render_proficiencies(&self, frame: &mut Frame, area: Rect) {
        ...
        frame.render_widget(
            Paragraph::new(text).block(Self::default_block().title(" proficiencies ")),
            area,
        );
    }
    ...
    fn render_notes(&mut self, frame: &mut Frame, area: Rect) -> Result<(), AppError> {
        ...
        if n_lines > area.height as usize {
            self.note_scrollbar_state = self
                .note_scrollbar_state
                .content_length(n_lines - (area.height - 1) as usize);
    ...
    fn render_tabs(&mut self, frame: &mut Frame, area: Rect) -> Result<(), AppError> {
        ...
        let area = area.offset(Offset::new(0, 1)).resize(layout::Size {
            width: area.width,
    ...
    fn render(&mut self, frame: &mut Frame) -> Result<(), AppError> {
        ...
        let [header, middle, bindings] =
            area.layout(&Layout::vertical([Length(6), Fill(1), Length(1)]).spacing(1));

        let [left, right] = middle.layout(&Layout::horizontal([Length(40), Fill(1)]).spacing(1));
